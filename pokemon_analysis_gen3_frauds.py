...
# 2. Filter for "Frauds" (Gen 3 only)
frauds = df[(df['is_legendary'] == 1) & (df['generation'] == 3) & (df['base_total'] < 600)]
...
for name in frauds['name']:
    if name == 'Regirock':
        colors.append('#A89030') # Rock Brown
    elif name == 'Regice':
        colors.append('#98D8D8') # Ice Blue
    elif name == 'Registeel':
        colors.append('#B8B8D0') # Steel Gray
    else:
        colors.append('gray')
...
# 5. Styling
plt.title("The Hall of Frauds: Gen 3", fontsize=16, fontweight='bold')
...
# 6. Save
plt.savefig('gen3_frauds.png', dpi=300)
plt.show()