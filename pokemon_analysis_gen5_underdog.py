...
# 2. Filter for "Underdogs" (Gen 5 only, Non-Legendary, BST >= 600)
underdogs = df[(df['is_legendary'] == 0) & (df['generation'] == 5) & (df['base_total'] >= 600)].copy()
...
for name in underdogs['name']:
    if name == 'Hydreigon':
        colors.append('#634A9F')  # Deep Dragon Purple
    else:
        colors.append('gray')
...
text_y_pos = len(underdogs) - 0.2 if len(underdogs) > 0 else 0
plt.text(605, text_y_pos, 'God Threshold (600)', fontsize=12, fontweight='bold', color='black')

# 5. Styling
plt.title("The Underdogs: Gen 5", fontsize=20, fontweight='bold', pad=20)
...
# 6. Save
plt.savefig('gen5_underdogs.png', dpi=300)
plt.show()